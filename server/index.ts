...
function isPortimport "dotenv/config";
import fs from "fs";
import express from "express";
import { createServer } from "http";
import net from "net";
...
async function startServer() {
  ...
  // Serve temporary audio uploads
  const uploadDir = process.env.UPLOAD_DIR ?? "/tmp/samploop";
...
startServer().catch(console.error);Available(port: number): Promise<boolean> {
  return new Promise(resolve => {
    const server = net.createServer();
    server.listen(port, () => { server.close(() => resolve(true)); });
    server.on("error", () => resolve(false));
