...
const allowedAudioMimeTypes = {
  ...
  "audio/x-wav": "wav",
  "audio/mpeg": "mp3",
  "audio/mimport { TRPCError } from "@trpc/server";
...
export const appRouter = router({
  ...
  auth: router({
    // Auth is disabled — always returns null, no redirect
  ...
  library: router({
    ...
    importBase64: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const ownerSlug = ownerUserId != null ? String(ownerUserId) : "anon";
        const stem = sanitizeFileStem(input.originalFileName || input.name) || "sample";
        const placeholderKey = `samploop/pending/${ownerSlug}/${crypto.randomUUID()}.${extension}`;
        ...
        try {
          ...
          throw new TRPCError({
            ...
            message:
              error instanceof Error
                ? `Échec de l'envoi du sample vers le stockage : ${error.message}`
                : "Échec de l'envoi du sample vers le stockage.",
          });
        }
...
const MAX_UPLOAD_BYTES = 24 * 1024 * 1024;
const allowedAudiimport { TRPCError } from "@trpc/server";
import { z } from "zod";
import { COOKIE_NAME } from "@shared/const";
import {
  createPendingAudioSample,
...
export type AppRouter = typeof appRouter;oMimeTypes = {
  "audio/wav": "wav",
  "audio/x-wav": "wav",
  "audio/mpeg": "mp3",
  "audio/mp3": "mp3",
...
export type AppRouter = typeof appRouter;p3": "mp3",
  "audio/ogg": "ogg",
  "audio/webm": "webm",
} as const;

const seeimport { TRPCError } from "@trpc/server";
import { z } from "zod";
import { systemRouter } from "./_core/systemRouter";
import { publicProcedure, router } from "./_core/trpc";
import { storagePut } from "./storage";
...
export type AppRouter = typeof appRouter;dedSamples = [
  { id: "a1", name: "Amber Kick Loop",    color: "#f59e0b", category: "Drums",   bpm: 120, durationMs: 8000,  isLoop: true, sourceKind: "seeded", fileUrl: "", fileKey: "seeded/amber-kick-loop.wav",    sortName: "amber kick loop",    mimeType: "audio/wav", byteSize: 0, dominantColor: "#f59e0b", waveformPreview: null, originalFileName: "amber-kick-loop.wav",    ownerUserId: null, libraryStatus: "ready" },
